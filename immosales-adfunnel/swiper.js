...
var Swiper = function() {
    ...
    const ue = [function(e) {
        ...
        function p(e) {
            const bulletClass = t.params.pagination.bulletClass;
            // Ensure bulletClass is not empty and starts with a dot
            const validBulletClass = bulletClass && bulletClass.trim() !== '' 
                ? (bulletClass.startsWith('.') ? bulletClass : `.${bulletClass}`)
                : '.swiper-pagination-bullet'; // fallback to default class
        
            const s = e.target.closest(validBulletClass);
            if (!s) return;
            e.preventDefault();
